...
time = 0:cfg.scenario.dt:cfg.scenario.t_end;
...
position(:, 1) = cfg.scenario.initial_state_ca([1, 4]);
velocity(:, 1) = cfg.scenario.initial_state_ca([2, 5]);
...
for k = 1:(num_steps - 1)
    ...
    position(:, k + 1) = position(:, k) ...
        + velocity(:, k) * cfg.scenario.dt ...
        + 0.5 * accel_now * cfg.scenario.dt^2;
    velocity(:, k + 1) = velocity(:, k) + accel_now * cfg.scenario.dt;
end
