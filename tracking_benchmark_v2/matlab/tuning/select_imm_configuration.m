...
candidates = build_imm_candidate_bank(cfg.algorithms.imm_cv);
num_candidates = numel(candidates);
stage1_trials = min(cfg.algorithms.imm_cv.tuning.stage1_trials, size(measurements.z, 3));
...
for idx = 1:num_candidates
    batch = run_imm_cv_batch(stage1_measurements, truth, candidates(idx), cfg);
    evaluated = evaluate_estimates(batch, truth, cfg);
    stage1_summaries(idx) = build_candidate_summary( ...
        idx, candidates(idx), evaluated.metrics, cfg, stage1_trials);
end

stage1_order = rank_candidates(stage1_summaries);
top_count = min(cfg.algorithms.imm_cv.tuning.full_eval_candidates, num_candidates);
...
for rank_index = 1:top_count
    ...
    batch = run_imm_cv_batch(measurements, truth, candidates(candidate_index), cfg);
    evaluated = evaluate_estimates(batch, truth, cfg);
    full_summaries(rank_index) = build_candidate_summary( ...
        candidate_index, candidates(candidate_index), evaluated.metrics, cfg, size(measurements.z, 3));
end
...
function candidates = build_imm_candidate_bank(base_imm)
...
num_candidates = numel(q_low_values) * numel(q_mid_values) * numel(q_high_values) * ...
    size(transition_bank, 3) * size(mu0_bank, 2) * numel(P0_scale_values);
base_imm.candidate_name = '';
base_imm.auto_tune = false;
base_imm.tuning = struct();
base_imm.enabled = true;
base_imm.model_labels = {'Low maneuver', 'Medium maneuver', 'High maneuver'};
if ~isfield(base_imm, 'x0')
    error('IMM base configuration is missing x0.');
end

candidates = repmat(base_imm, num_candidates, 1);
candidate_index = 0;
